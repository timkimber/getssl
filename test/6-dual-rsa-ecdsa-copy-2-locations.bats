...
setup() {
    ...
    curl -X POST -d '{"host":"'a.$GETSSL_HOST'", "addresses":["'$GETSSL_IP'"]}' http://10.30.50.3:8055/add-a
}


teardown() {
    curl -X POST -d '{"host":"'a.$GETSSL_HOST'", "addresses":["'$GETSSL_IP'"]}' http://10.30.50.3:8055/del-a
}


@test "Create dual certificatse and copy RSA and ECDSA chain and key to two locations" {
